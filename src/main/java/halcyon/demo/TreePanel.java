...
public class TreePanel extends ControlWindowBase
{
	...
	private final Node rootIcon = new ImageView(new Image(TreePanel.class.getResourceAsStream(ResourcesUtil.getString("root.icon"))));
	...
	public TreePanel()
	{
		...
		for (HalcyonNodeType type : HalcyonNodeTypeExample.values())
		{
			String iconName = ResourcesUtil.getString(type.name().toLowerCase() + ".icon");
